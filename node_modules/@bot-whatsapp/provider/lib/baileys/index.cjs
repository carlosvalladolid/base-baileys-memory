    ...
    busEvents = () => [
        {
            ...
            func: ({ messages, type }) => {
                ...
                console.log("Type: " + messageCtx?.message?.protocolMessage?.type)
                ...
                console.log(JSON.stringify(messageCtx, null, 2))
                
                console.log("messageCtx?.message: ")
                console.log(JSON.stringify(messageCtx?.message, null, 2))
                ...
                console.log("fromMe: " + payload?.key?.fromMe)

                if (payload?.key?.fromMe) {
                    console.log("Return fromMe")
                    return
                }
                ...
                console.log("BlistRowIddy2: " + listRowId)

                // ***
                // Esta es la parte buena del código, se ponen varias condiciones, porque por alguna razón WhatsApp manda diferentes
                // estructuras en el JSON del mensaje, si de repente el programa no funciona, probablemente sea porque se cambió esta estructura
                // y hay que poner otra condición, dejo las anteriores por si cambia entre diferentes números de Carbyne
                // Última modificación 13 Marzo 2025
                if (payload.body == '' || payload.body == null) {
                    console.log("Body1: " + messageCtx?.message?.templateMessage?.interactiveMessageTemplate?.body.text)
                    payload.body = messageCtx?.message?.templateMessage?.interactiveMessageTemplate?.body.text;
                }
                // Última modificación 11 Julio 2024
                if (payload.body == '' || payload.body == null) {
                    console.log("Body2: " + messageCtx?.message?.templateMessage?.hydratedTemplate?.hydratedContentText)
                    payload.body = messageCtx?.message?.templateMessage?.hydratedTemplate?.hydratedContentText;
                }

                if (payload.body == '' || payload.body == null) {
                    console.log("Body3: " + messageCtx?.message?.highlyStructuredMessage?.localizableParams[1]?.default)
                    payload.body = messageCtx?.message?.highlyStructuredMessage?.localizableParams[1]?.default;
                }

                if (payload.body == '' || payload.body == null) {
                    console.log("Body4: " + messageCtx?.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate?.hydratedContentText)
                    payload.body = messageCtx?.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate?.hydratedContentText;
                }
                // ***
